...
def build_email(dispatch: dict, meta: dict) -> tuple[str, str]:
    ...
    url = html.escape(meta["base_url"], quote=True).rstrip("/")
    # Masthead as an image so the blackletter renders in email (webfonts do not).
    masthead = (f'<img src="{url}/assets/email-masthead.png" alt="The Aftertimes" '
                f'width="300" style="display:block;margin:0 auto 8px;width:300px;'
                f'max-width:78%;height:auto;">')
    # The engraving, from an absolute URL (relative img paths do not load in email).
    img = dispatch.get("image")
    engraving = (
        f'<img src="{url}/{html.escape(img, quote=True)}" alt="An imagined engraving" '
        f'style="display:block;width:100%;height:auto;margin:0 0 18px;">'
        if img else "")
    body = f"""<!DOCTYPE html>
...
<tr><td style="padding:28px 32px;font-family:Georgia,serif;">
  <div style="text-align:center;border-bottom:3px double #1a1611;padding-bottom:12px;margin-bottom:20px;">
    {masthead}
    <div style="font-size:10px;letter-spacing:0.24em;text-transform:uppercase;color:#6b5f4d;font-family:Arial,sans-serif;">{tagline}</div>
  </div>
