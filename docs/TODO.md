...
- [ ] Custom domain: `site.base_url` in settings.yaml now reads `https://aftertimes.charlietrenorden.com`. Confirm the domain is live and the GitHub Pages CNAME is configured - the NEWSLETTER builds its links and masthead image URL from `base_url`, so if the domain is not serving, every daily email points at a dead host.
